...
function getPostIdFromElement(postEl) {
  ...
  const link = postEl.querySelector('a[href*="wall"]');
  if (link) {
...
function processPosts(root = document) {
  const posts = root.querySelectorAll('div[id^="post-"], [data-post-id]');
...
const observer = new MutationObserver((mutations) => {
  for (const mutation of mutations) {
    mutation.addedNodes.forEach((node) => {
      ...
      if (node.matches && (node.matches('div[id^="post-"]') || node.matches('[data-post-id]'))) {
